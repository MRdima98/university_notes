...
\begin{enumerate}
  ...
  \item $\delta_N(q, i, Z) = \{(q,ZZ)\}$ leggo i aggiungo Z
  \item $\delta_N(q, e, Z) = \{(q,\epsilon)\}$ leggo e rimuovo z
  \item $\delta_N(q, \epsilon, X_0) = \{(r,\epsilon)\}$ $P_F$ accetta quando $P_N$ si svuota
\end{enumerate}
