...
\newcommand{\rombo}[0]{%
  \begin{tikzpicture}
      \draw (0,0) -- (0.10,0.20) -- (0.20,0) -- (0.10,-0.20) -- cycle; 
  \end{tikzpicture}%
}
...
$\epsilon$-NFA per riconoscere un numero decimale
	...
	\end{enumerate}
	\textbf{Teorema} Un linguaggio L è accetto da un $\epsilon$-NFA se è solo se è accettato da un DFA.
...
\\ Un linguaggio che ha una CFG è detto \emph{libero da contesto}.
...
È possibile, in alcuni casi, rimuovere l'ambiguità. Non esiste però un modo sistematico e alcuni CFL(context free language) hanno solo CFG ambigue. 
...
\end{enumerate}
Se un linguaggio ha un PDA accettante, allora è un CFL.
...
\\ The two rules are:
\begin{itemize}
    \item lookahead for shift: only on the expected terminals
...
\\ Logical consequence: 
\begin{itemize}
    \item Formal $\psi$ is a logical consequence of $\phi_1,...,\phi_n$ (this formulas are a series of premises) if for any M, when M $\models \phi_1,...,\phi_n$ then M $\models \psi$
    \item This form is equivalent to: $\phi_1,...,\phi_n \vdash \psi$ ( this would be a series of premises that leads to a conclusion, and it's called a \emph{sequent})
\end{itemize}
...
\begin{itemize}
    ...
    \item Form for rules: $ \inferrule
            ...
            { \vdash \phi} $
            \\ premises above and conclusion below
\end{itemize}
...
\begin{itemize}
    ...
    \item 
        \begin{tikzpicture}
            \draw (0,0) -- (0.15,0.3) -- (0.3,0) -- (0.15,-0.3) -- cycle; 
        \end{tikzpicture}
        $\phi: \phi$ è vera in alcuni momenti successivi
    \item $\phi \; \mathcal{U} \psi$: $\phi$ è vera fino a quando $\psi$ vera
\end{itemize}
Un LTL è una serie di stati che si estende nel futuro. Questo viene chiamato \emph{path},
siccome il futuro non è deciso ci sono una serie di path prima di trova quello corretto.
\\ Sintassi LTL: 
\\ $\phi, \psi \rightarrow$ p $|$ (atomic proposition) 
\\ \hspace*{5mm} T $|$ (true)
\\ \hspace*{5mm} $\bot |$ (false)
\\ \hspace*{5mm} $\neg \phi |$ (complement)
...
\\ \hspace*{5mm} 
        \begin{tikzpicture}
            \draw (0,0) circle (0.15);
        \end{tikzpicture} (next time)
\\ \hspace*{5mm} 
        \begin{tikzpicture}
            \draw (0,0) rectangle (0.3, 0.3);
        \end{tikzpicture} (always)
\\ \hspace*{5mm} 
        \begin{tikzpicture}
            \draw (0,0) -- (0.15,0.3) -- (0.3,0) -- (0.15,-0.3) -- cycle; 
        \end{tikzpicture} (sometime)
\\ \hspace*{5mm} 
...
\end{itemize}
L'operatore next indica la proposizione corretto nel prossimo momento temporale. Esempio: (sad $\land \neg$ rich) \mycircle sad. La semantica corretta sarebbe: $\pi \models \mycircle \Longleftrightarrow \pi^2 \models \phi$. ($\pi_i$ è la sequenza di mondi, quindi al quadrato significa solo che è il secondo mondo)
\vspace{5mm}
\\ L'operatore sometime è vero ora o in qualche momento futuro. Esempio: sad $\Rightarrow$ \rombo happy. Semantica: $\pi \models \rombo \phi \Longleftrightarrow$ there is some i $\geq$ 1 such that $\pi_1 \models \phi$.
\vspace{5mm}
\\ L'operatore always è vero ora e in ogni momento futuro. Esempio: lotter-win $\Rightarrow$ \square rich. Semantica: $\pi \models \square \; \phi \Longleftrightarrow$ for all i $\geq$ 1 we have $\pi^i \models \phi$.
\vspace{5mm}
\\ L'operatore until mettere in relazione due proposizioni, una vera ora e per tutti i momenti successivi finché non è vera un altra. Esempio born $\Rightarrow$ alive $\mathcal{U}$ dead. Semantica: $\pi \models \phi \mathcal{U} \psi \Longleftrightarrow$ there is some i $\geq$ 1 such that $\pi^i \models \psi$ and for all j = 1...i-1 we have $\pi^j \models \psi$.
\vspace {5mm} 
\\ Possiamo minimizzare gli operatori: 
\begin{itemize}
    \item always e sometime sono duali: $\neg \square{0.30} \phi \equiv \rombo \neg \psi$
    \item sometime può essere espresso usando until: $\square \phi \equiv T \mathcal{U} \psi$
    \item quindi tutti gli operatori si possono esprimere tramite next e until
\end{itemize}
Relazione con operatori classici: 
\begin{itemize}
    \item sometime distribuisce su or e always su and 
        \\ $\rombo \; (\phi \lor \psi) \equiv \rombo \phi \lor \rombo \psi$
        \\ $\square \; (\phi \land \psi) \equiv \square \phi \land \square \psi$
    \item relazione con il not: 
        \\ $\neg \; \mycircle \; \phi \equiv \mycircle \; \neg \phi$
        \\ $\neg (\psi \mathcal{U} \phi) \equiv (\neg \psi \mathcal{U} (\neg \phi \land \neg \psi)) \lor \; \square \; \neg \psi$
\end{itemize}
Possiamo usare notazione alternative: 
\begin{itemize}
    \item $\rombo \; \rightarrow$ F oppure $<>$
    \item $\square \; \rightarrow$ G oppure $[]$
    \item $\mycircle \; \rightarrow$ X oppure $()$
\end{itemize}
Ecco alcuni concetti importanti per i sistemi informatici: 
\begin{itemize}
    \item safety (\square $\neg$ ....): non raggiungo mai stati con errori 
    \item liveness (\rombo ....): primo o poi eseguo un azione
    \item fairness (\square \; \rombo \; ready $\Rightarrow$ \square \; \rombo \; run): se chiedo una cosa infinite volte, verrà eseguita infinite volte
\end{itemize}
Le proposizioni $\pi_i$(p) per i $\geq$ 1 sono codificati tramite predicati unari p(i) su numeri naturali. Una formula LTL $\phi$ è codificata dalla formula $[\![\phi]\!]_{s(z)}$, dove: 
\\ $[\![p]\!]_t$ = p(t)
\\ $[\![\mycircle \phi]\!]_t$ = $[\![\phi]\!]_{s(t)}$
\\ $[\![\rombo \phi]\!]_t$ = $ \exists t' \; t' \geq t \land [\![\phi]\!]_{t'}$
\\ $[\![\square \phi]\!]_t$ = $ \forall t' \; t' \geq t \rightarrow [\![\phi]\!]_{t'}$
\\ $[\![\phi \mathcal{U} \psi]\!]_t$ = $ \forall t' \; t' \geq t \rightarrow [\![\psi]\!]_{t'} \land \forall t''(t \leq t'' \land s(t'') \leq t') \rightarrow [\![\phi]\!]_{t''}$
